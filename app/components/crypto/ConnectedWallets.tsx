...
import type { ConnectedAccount, CryptoAllocationDetail } from "../../lib/mock/portfolioContext";
...
function WalletCard({
  ...
  return (
    ...
    >
      ...
      {account.networks && account.networks.length > 0 && (
...
function WalletHoldingsModal({
  account,
  holdings,
  onClose,
}: {
  account: ConnectedAccount;
  holdings: CryptoAllocationDetail[];
  onClose: () => void;
}) {
  useEffect(() => {
...
export default function ConnectedWallets({
  ...
  allocations: CryptoAllocationDetail[];
}) {
    ...
    <div>
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        {accounts.map((account) => (
          <WalletCard key={account.id} account={account} onSelect={setSelectedId} />
        ))}
      </div>
      ...
      {selectedAccount && (
        <WalletHoldingsModal
          ...
          holdings={allocations.filter((row) => row.accountId === selectedAccount.id)}
