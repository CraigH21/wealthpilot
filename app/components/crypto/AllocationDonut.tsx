...
export default function AllocationDonut({ slices, total }: { slices: AllocationSlice[]; total: number }) {
  ...
  return (
    ...
    >
      <EdgeGlow />
      <h3 className="text-sm font-semibold text-zinc-900">Portfolio Allocation</h3>
      ...
      <div className="mt-4 flex items-center gap-5">
        <div className="relative h-32 w-32 shrink-0">
          ...
          <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
            <span className="text-sm font-semibold text-zinc-900">{formatGBP(total)}</span>
            <span className="text-[11px] text-zinc-600">Total Value</span>
          </div>
        </div>
        ...
        <div className="flex flex-1 flex-col gap-2">
          {slices.map((slice) => (
            <div key={slice.symbol} className="flex items-center justify-between gap-2 text-xs">
              ...
              <span className="font-medium text-zinc-600">{slice.pct}%</span>
            </div>
          ))}
        </div>
      </div>
