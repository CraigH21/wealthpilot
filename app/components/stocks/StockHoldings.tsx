...
export default function StockHoldings({ holdings }: { holdings: StockHoldingRow[] }) {
  ...
  return (
    <div className="glass-edge-card relative overflow-hidden rounded-3xl p-6 shadow-[0_8px_32px_rgba(0,0,0,0.45),inset_3px_3px_8px_rgba(255,255,255,0.1),inset_-2px_-2px_6px_var(--accent-soft)] backdrop-blur-[32px] sm:p-8">
      ...
      <div className="relative flex items-center justify-between">
        <div>
          <h2 className="text-base font-semibold text-zinc-900">My Stock Holdings</h2>
      ...
      {/* Tall enough for all 6 mock holdings — thead/tfoot stay pinned via
          sticky positioning so column labels and the total stay visible
          while any extra holdings beyond that scroll underneath them. */}
      <div className="relative mt-5 max-h-[420px] overflow-auto">
        <table className="w-full min-w-[900px] border-collapse text-left text-sm">
          <thead className="sticky top-0 z-10">
            <tr className="text-xs uppercase tracking-wide text-zinc-600">
              <th className="pb-3 pr-2 font-medium">#</th>
              <th className="pb-3 pr-2 font-medium">Stock / ETF</th>
                ...
                <tr key={holding.symbol} className="transition-colors duration-300 ease-out hover:bg-black/5">
                  ...
                  <td className="py-3 pr-2">
                    <div className="flex items-center gap-2.5">
                      {holding.logoSymbol ? (
                        <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-black/5 p-1">
                  ...
                  <td className="py-3 pr-2 text-right text-zinc-600">
                    {holding.quantity !== null ? `${holding.quantity.toFixed(2)} ${holding.symbol}` : "—"}
                  </td>
                  <td className="py-3 pr-2 text-right text-zinc-600">
