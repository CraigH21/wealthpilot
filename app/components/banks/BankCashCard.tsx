...
export default function BankCashCard({
  totalCash,
  netThisMonth,
  monthlyIncome,
  monthlySpending,
          ...
          <div className="mt-2 flex flex-wrap items-end gap-3">
            ...
            <span
              className={`mb-1 inline-flex items-center gap-1 rounded-full px-3 py-1 text-sm font-medium ${
                netPositive ? "bg-accent-soft text-accent" : "bg-red-500/10 text-red-400"
              }`}
            >
              {netPositive ? "+" : ""}
              {formatGBP(netThisMonth)} this month
            </span>
          </div>

          <div className="mt-4 flex flex-wrap gap-6">
            <div>
              <p className="text-[11px] font-medium uppercase tracking-wide text-zinc-600">Monthly Income</p>
              <p className="mt-0.5 text-sm font-semibold text-zinc-900">{formatGBP(monthlyIncome)}</p>
            </div>
            <div>
              <p className="text-[11px] font-medium uppercase tracking-wide text-zinc-600">Monthly Spending</p>
              <p className="mt-0.5 text-sm font-semibold text-zinc-900">{formatGBP(monthlySpending)}</p>
            </div>
            <div>
              <p className="text-[11px] font-medium uppercase tracking-wide text-zinc-600">Net This Month</p>
              <p className={`mt-0.5 text-sm font-semibold ${netPositive ? "text-accent" : "text-red-400"}`}>
                {netPositive ? "+" : ""}
                {formatGBP(netThisMonth)}
              </p>
            </div>
          </div>
        </div>
